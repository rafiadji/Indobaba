...
class M_admin_produk extends CI_Model {
	...
	function ubahtonull($data)
	{
		$sql = $this->db->update('mp_produk', $data);
	}
	
	function ubahSts($id_produk, $sts_lama)
	{
		if($sts_lama == 0){$sts_br = 1;}elseif($sts_lama == 1){$sts_br = 0;}
		$sql = $this->db->where(array("ID_PRODUK" => $id_produk))
				->update("mp_produk", array("STS_PUBLISH" => $sts_br));
	}
