<?php
class M_user_riwayat_saldo extends CI_Model{
	...
	function outdata($table, $akun, $sts, $group, $sts1)
	{
		$out = $this->db->query('SELECT * FROM '.$table.' WHERE (ID_STATUS = '.$sts.' OR ID_STATUS = '.$sts1.')  AND ID_AKUN = '.$akun.' GROUP BY '.$group.' ORDER BY NO_TRANS DESC');
		return $out->result();
	}
}