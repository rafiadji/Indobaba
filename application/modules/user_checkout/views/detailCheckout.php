	...
	<div class="row">
		<div class="col-md-12">
    		...
    		<div class="col-md-12">
    			...
    			<?php if($stat->STS_BAYAR == 1):?>
    				<?php if($stat->ID_STATUS == 2 || $stat->ID_STATUS == 3):?>
	    			<h4>Pembayaran Sudah Di Konfirmasi</h4>
	    			<br/>
	    			Anda telah melakukan pembayaran, silahkan tunggu 1x24 jam untuk dikonfirmasi oleh admin indobaba. Jika sudah dikonfirmasi silahkan cek di <strong>"Transaksi Anda"</strong> di status <strong>"Orderan Ditanggapi"</strong>
    				<?php elseif($stat->ID_STATUS == 4):?>
	    			<h4>Pembayaran Sudah Di Konfirmasi</h4>
	    			<br/>
	    			Anda telah melakukan pembayaran, silahkan tunggu 1x24 jam untuk dikonfirmasi oleh admin indobaba. Jika sudah dikonfirmasi silahkan cek di <strong>"Transaksi Anda"</strong> di status <strong>"Orderan Ditanggapi"</strong>
    				<?php elseif($stat->ID_STATUS == 5):?>
	    			<h4>Pembayaran Tidak Valid</h4>
	    			<br/>
	    			Pembayaran yang anda lakukan belum diterima oleh admin Indobaba, Silahkan cek pembayaran anda dan masukkan bukti pembayaran lagi
	    			<?php elseif($stat->ID_STATUS == 9):?>
	    			<h4>Transaksi Telah Kadaluwarsa</h4>
	    			<br/>
	    			Transaksi telah kadaluwarsa karena anda tidak membayar selama 3 hari
	    			<?php else:?>
    				<h4>Pembayaran Sudah Di Konfirmasi</h4>
	    			<br/>
	    			Anda telah melakukan pembayaran, silahkan tunggu 1x24 jam untuk dikonfirmasi oleh admin indobaba. Jika sudah dikonfirmasi silahkan cek di <strong>"Transaksi Anda"</strong> di status <strong>"Orderan Ditanggapi"</strong>
    				<?php endif;?>
    			<?php elseif($stat->STS_BAYAR == 0): ?>
    				<?php if($stat->ID_STATUS == 9):?>
	    			<h4>Transaksi Telah Kadaluwarsa</h4>
	    			<br/>
	    			Transaksi telah kadaluwarsa karena anda tidak membayar selama 3 hari
	    			<?php else:?>
	    			...
	    			<br/>
	    			Silahkan kirim biaya total belanja ke nomor rekening di bawah ini. Jika sudah melakukan pembayaran silahkan konfirmasi dengan klik tombol <strong>"Bukti Pembayaran"</strong>
    				<?php endif;?>
    			<?php endif;?>
    			</div>
    		</div>
    			...
    			<div class="col-md-2">
    				<div class="panel panel-default">
    					<div class="panel-body"style="text-align: center;height: 250px;">
    						<img src="<?php echo base_url($row_rekening->GAMBAR) ?>" width="200px">
					<p style="font-weight: bold;margin-top:20px;"><?php echo $row_rekening->BANK;?></p>
					<p class="rekening-bank"><?php echo $row_rekening->NO_REKENING;?></p>
					<p class="a/n-bank">a/n <?php echo $row_rekening->ATAS_NAMA;?></p>
    					</div>
    				</div>
    			</div>
    			<?php endforeach; ?>
		...
		<?php if($stat->STS_BAYAR == 1 &&  $stat->STS_SALDO == 0):
				$k = 0;
	            foreach ($toko as $tk) {
	            	$m = 0;
			...
			<div class="panel panel-success">
				...
				<table class="table table-striped" >
					<tr>
						...
						<td>:</td>
						<td><p><?=$bukti->TGL_TRANSFER?></p></td>
					</tr>
					<tr>
						<th>No Rekening Pengirim</th>
						<td>:</td>
						<td><p><?=$bukti->NO_REKENING?></p></td>
					</tr>
					<tr>	
						<th>Atas Nama Pengirim</th>
						<td>:</td>
						<td><p><?=$bukti->ATAS_NAMA?></p></td>
					</tr>
					<tr>	
						<th>Nama Bank Tujuan</th>
						<td>:</td>
						<td><p><img src="<?=site_url($bukti->GAMBAR)?>" width="150" /></p></td>
					</tr>
					<tr>
						<th>Total Transfer</th>
						<td>:</td>
						<td><p><?php echo formatRp($tot_tag[$l-1] + $tk->KODE_UNIK); ?></p></td>
					</tr>
				</table>
				</div>
			</div>
		...
		<?php endif; ?>
		<?php if($this->session->flashdata('notifi')): ?>
		<div class="col-md-12">
			<div class="alert alert-<?php echo $this->session->flashdata('clri');?> alert-dismissable">
				<i class="fa fa-check"></i>
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<?php echo $this->session->flashdata('notifi'); ?>
			</div>
		</div>
		<?php endif; ?>
		<div class="col-md-9">
